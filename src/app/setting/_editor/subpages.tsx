...
// 부모 SubPage 내부의 children 관리. 자식은 부모 slug 내 유니크만 요구.
// 자식은 별도 헤더 메뉴 등록 안 함 (부모 메뉴가 대표). 라이브 사이트에서 부모
// 메뉴 hover 시 드롭다운 노출 여부는 라이브 사이트 렌더링 작업의 몫.
export function ChildPagesEditor({
    parent,
    onChange,
    currentPageId,
    onSelectPage,
    ...
    parent: SubPage;
    onChange: (nextChildren: SubPage[]) => void;
    ...
    const add = () => {
        ...
        if (items.some((c) => c.slug === s)) {
            toast.show("이미 존재하는 하부메뉴 주소입니다.", "error");
            return;
        }
        onChange([...items, { id: uid(), slug: s, title: t, sections: [] }]);
        setSlug("");
        setTitle("");
        toast.show(`/${parent.slug}/${s} 하부메뉴가 추가되었습니다.`);
    };

    const remove = async (c: SubPage) => {
        const ok = await confirm({
            title: `/${parent.slug}/${c.slug} 하부메뉴 삭제`,
            message: "이 하부메뉴와 모든 섹션이 삭제됩니다. 되돌릴 수 없습니다.",
            confirmLabel: "삭제",
            danger: true,
        });
        if (!ok) return;
        if (currentPageId === c.id) onSelectPage(parent.id);
    ...
    return (
        <div>
            <div className="text-[11px] text-slate-500 mb-2">
                <span className="font-mono">/{parent.slug}</span> 아래 하부메뉴.
                예: <code>84a</code> → <code>/{parent.slug}/84a</code>
            </div>
                            ...
                            >
                                <div className="flex items-center gap-0.5 shrink-0">
                                    ...
                                    <input
                                        ...
                                        value={c.slug}
                                        onChange={(e) =>
                                            patch(c.id, { slug: e.target.value })
                                        }
                                        onBlur={(e) =>
                                            normalizeSlug(c.id, e.target.value)
                                        }
                                    />
                                </div>
                                <input
                                ...
                                <button
                                    ...
                                    onClick={() =>
                                        onSelectPage(active ? parent.id : c.id)
                                    }
                                >
                                    {active ? "편집 중" : "디자인 편집"}
                                </button>
